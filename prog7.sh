...
destination="./"
cp -R /localdisk/data/BPSM/ICA1/fastq/*.fq.gz $destination
cp -R /localdisk/data/BPSM/ICA1/TriTrypDB-46_TcongolenseIL3000_2019.bed $destination
cp -R /localdisk/data/BPSM/ICA1/Tcongo_genome/TriTrypDB-46_TcongolenseIL3000_2019_Genome.fasta.gz $destination
cp -R /localdisk/data/BPSM/ICA1/fastq/*.fqfiles $destination
...
#This needs to be a loop so...
 ...
 for f in *1.fq.gz
    ...
    bowtie2 --no-unal -p 16 -x congogenome -1 ${base}_1.fq.gz -2 ${base}_2.fq.gz | samtools view -b -o ${base}.bam -
    done
...
#The below for loop can be used to do this for each individual file. However, I am attempting to count intersects within each group.

#for b in *.bam
#    do 
#    base=$(basename $b ".bam")
#    echo "Counting alignments in ${base}"
#    bedtools intersect -a TriTrypDB-46_TcongolenseIL3000_2019.bed -b ${base}.bam -c > ${base}.txt
#    done
